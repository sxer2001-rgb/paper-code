...
# 设置工作目录
setwd("G:/shuinengliang/SEM/SU")

# 读取 Excel 文件
ES <- read.csv("全部数据719.csv")

library(plspm)
library(ggplot2)
library(reshape)
library(igraph)
data<- read.csv("全部数据719.csv", header = TRUE)
colnames(data)
...
WYESDR <- c(1, 1, 1, 1, 1, 0)

latent_path_r <- rbind(dixin,qihou,jinji,NDVI,landscape,WYESDR)
...
write.csv(Sample_pls_summary_text,"结果数据EY.csv")
