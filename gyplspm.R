...
library(reshape)
library(igraph)
data<- read.csv("全部数据710.csv", header = TRUE)
colnames(data)
...
GYESDR <- c(1, 1, 1, 1, 1, 0)

latent_path_r <- rbind(dixin,qihou,jinji,NDVI,landscape,GYESDR)
...
write.csv(Sample_pls_summary_text,"结果数据GY.csv")